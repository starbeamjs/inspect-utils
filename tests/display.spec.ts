...
import { Display, DisplayTuple, DisplayUnit, STYLE_NAMES } from "inspect-utils";
import { describe, expect, test } from "vitest";

describe("Display (low level)", () => {
  for (const name of STYLE_NAMES) {
    describe("Display(format)", () => {
      test(`f.${name}`, () => {
        assert(
          Display((f) => f[name]("Hello")),
          `Hello`,
        );
      });
      ...
      describe("Display(name, format)", () => {
        ...
        test(`description`, () => {
          assert(
            Display("Hello", (f) => f[name]("world"), { description: "short" }),
            `Hello[short](world)`,
          );
        });
        ...
        test(`annotation`, () => {
          assert(
            Display("Hello", (f) => f[name]("world"), { annotation: "short" }),
            `Hello(world short)`,
          );
        });
      });
